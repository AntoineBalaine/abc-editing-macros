...
exports.consolidateRestsInChordTransform = exports.reorderChordTransform = void 0;
const dispatcher_1 = require("./dispatcher");
const transformPitches_1 = require("./transformPitches");
const orderOfNotes = "cdefgab".split("");
const reorderChordTransform = (chord) => {
    //split ${chord} into a json array
    const strNotes = (0, dispatcher_1.noteDispatcher)({
        text: chord,
        context: { pos: 0 },
        transformFunction: (note) => `"${note}",`,
    });
...
exports.reorderChordTransform = reorderChordTransform;
const consolidateRestsInChordTransform = (chord) => {
    let chordPitches = chord
        .split("")
        .filter((e) => !(0, dispatcher_1.isRhythmToken)(e))
        .filter((n) => (0, dispatcher_1.isPitchToken)(n));
    let rhythmTokens = chord.split("").filter((e) => (0, dispatcher_1.isRhythmToken)(e));
...
exports.consolidateRestsInChordTransform = consolidateRestsInChordTransform;
