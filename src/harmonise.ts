...
import { dispatcher, isLowerCase } from "./transpose";
...
export const reorderChord = (chord: chordText) =>{
    ...
    const strNotes = dispatcher(chord, {pos:0},(note: abcText)=>`"${note}",`)
        ...
        if (note.includes(",")){
            ...
            if(foundMatchingOctave){ 
                for ( let i=0; i<octaveOrder["4apostrophe"].length; i+=1 ){
                    if (howMany(octaveOrder["4apostrophe"][i][0], `'`)===numberOfApostrophes){
    ...
    return "[" + [ 
            octaveOrder["1comma"].map(subArray=>subArray.sort((a,b)=>orderOfNotes.indexOf(a.toLowerCase()) - orderOfNotes.indexOf(b.toLowerCase()))),
            octaveOrder["2upperCase"].sort((a,b)=>orderOfNotes.indexOf(a.toLowerCase()) - orderOfNotes.indexOf(b.toLowerCase())),
            octaveOrder["3lowerCase"].sort((a,b)=>orderOfNotes.indexOf(a.toLowerCase()) - orderOfNotes.indexOf(b.toLowerCase())),
            octaveOrder["4apostrophe"].map(subArray=>subArray.sort((a,b)=>orderOfNotes.indexOf(a.toLowerCase()) - orderOfNotes.indexOf(b.toLowerCase()))),
    ].flat().join("") + "]";
