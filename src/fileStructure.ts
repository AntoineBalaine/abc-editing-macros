import { abcText, InstrumentCalls, instrumentFamilies } from "./annotationsActions";
import { contextObj, isNomenclatureLine } from "./transpose";
...
export const addNomenclatureToHeader = (headerText:string, tags: string[]): string =>{
    tags.forEach(tag=>headerText+=`\nV: ${tag} name="${tag}"`)
    return headerText;

};

export const buildBodyFromInstruments = (bodyText: InstrumentCalls[]): string =>{
    //split each instrument call into lines
    //[{tag: [lines]}, {tag: [lines]}]
    bodyText = bodyText.map(( instrumentCall:any )=>{
        Object.keys(instrumentCall).forEach(instrumentKey=>instrumentCall[instrumentKey] = instrumentCall[instrumentKey].split("\n"));
